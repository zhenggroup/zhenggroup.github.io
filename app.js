...
(() => {
  ...
  function renderPublications() {
    ...
    grid.innerHTML = selected.map(p => {
      ...
      return `${eraHeading}<article class="publication-card${p.image ? '' : ' no-image'}" id="paper-${number}" aria-labelledby="paper-title-${number}">${p.image ? `<button type="button" class="publication-image toc-open" data-paper-number="${number}" aria-haspopup="dialog" aria-label="${escape((language === 'zh' ? '放大 TOC 图：' : 'Enlarge TOC: ') + plain(p.title))}"><img src="${safeURL(p.image)}" alt="${escape(p.imageAlt || plain(p.title))}" loading="lazy" width="500" height="280"></button>` : ''}<div class="publication-copy" id="${escape(legacyAnchor)}"><span class="publication-number">${number}.</span><h3 id="paper-title-${number}"><a href="${escape(doi)}" target="_blank" rel="noopener noreferrer">${inlineHTML(p.title)}</a>${p.type ? ` <span class="paper-type">(${escape(p.type.toLowerCase())})</span>` : ''}</h3><p class="paper-authors">${inlineHTML(p.authors)}</p><p class="paper-citation">${inlineHTML(p.journal)}</p>${p.comments?.length ? `<div class="paper-notes">${p.comments.map(note=>`<p>${escape(note)}</p>`).join('')}</div>` : ''}${p.featuredIn?.length ? `<div class="paper-related">${p.featuredIn.map(link=>`<a href="${safeURL(link.url)}"${external(link.url)}>${escape(link.text)} ↗</a>`).join('')}</div>` : ''}<div class="paper-bottom"><span class="paper-identifiers"><span class="dimensions-badge-inline" aria-label="Dimensions citations"><span class="__dimensions_badge_embed__" data-doi="${escape(badgeDoi)}" data-style="small_rectangle" data-legend="hover-bottom"></span></span></span>${p.pdfLink ? `<a href="${safeURL(p.pdfLink)}" target="_blank" rel="noopener noreferrer">PDF ↗</a>` : ''}</div></div></article>`;
    }).join('');
