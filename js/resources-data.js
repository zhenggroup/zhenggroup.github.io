// Local fallback for file:// previews.
// Keep this content synchronized with resource/resources.md.
window.resourceMarkdownFallback = String.raw`# Resources

## Intro

Title: Tools and Learning Materials

Quick access to lab systems, recommended scientific software, group-developed tools, and selected electrochemistry learning materials.

## Lab Systems

> **For Zheng Lab members**
> Use the lab systems below for instrument scheduling and shared data access.

### Instrument Booking

Icon: calendar-alt
URL: https://el.easylab.xyz/lab/zhenggroup
Style: booking

Reserve lab equipment and manage schedules.

### NAS Data & Files

Icon: server
Simple Icon: synology
URL: https://zheng-lab-gtiit.cn5.quickconnect.cn/
Style: nas

Access shared group files and research data.

## Software Recommendations

### Python

URL: https://www.python.org/
Simple Icon: python
Note: Free
Featured: true

Essential programming language for data processing, plotting, system simulation, and hardware control in electrochemistry research.

- **Core Packages:** NumPy, Pandas, SciPy, Matplotlib, Seaborn for scientific computing and visualization.
- **Recommended Environment:** [Visual Studio Code](https://code.visualstudio.com/) with [Codex](https://openai.com/codex/) for AI-assisted coding, data analysis, and documentation.
- **Why Python:** Read [this article](https://iopscience.iop.org/article/10.1149/2754-2734/acff0b/meta) explaining our preference for Python over OriginLab or Excel.
- **Learning Resource:** View Dr. Zheng's ECS Webinar on Python for electrochemistry [here](https://physicsworld.com/a/introducing-python-for-electrochemistry-research/).

### Blender

URL: https://www.blender.org/
Simple Icon: blender
Note: Free

Professional 3D modeling, animation, rendering, and visual effects software for creating scientific visualizations.

### Fiji (ImageJ)

URL: https://fiji.sc/
Simple Icon: imagej
Note: Free

Open-source image processing software specialized for scientific image analysis, particularly useful for electron microscopy images.

### VESTA

URL: https://jp-minerals.org/vesta/en/
Note: Free

3D visualization software for crystal structures, volumetric data, and crystal morphologies in materials science.

### Gwyddion

URL: https://gwyddion.net/
Note: Free

Modular software for scanning probe microscopy (SPM) data analysis and visualization.

## Lab-Developed Tools

### VersaParse

URL: https://github.com/zhenggroup/versaparse
Simple Icon: github
Note: Free, Open Source

Python package for parsing and processing .PAR files from electrochemical instruments.

- **Purpose:** Automated parsing of PAR electrochemical data files for integration into Python workflows.
- **Installation:** Available on <a class="brand-icon-link" href="https://github.com/zhenggroup/versaparse"><span class="simple-icon simple-icon-github" aria-hidden="true"></span>GitHub</a>.

## Electrochemical Learning Resources

### Recent Lectures

**电催化剂失活过程的原位研究策略** (In Situ Study Strategies for Electrocatalyst Deactivation Processes)
Researchcloud（科研云） by Weiran Zheng • Feb. 6, 2025
<a class="brand-icon-link" href="https://www.bilibili.com/video/BV1bzN4efEDM/"><span class="simple-icon simple-icon-bilibili" aria-hidden="true"></span>View on Bilibili</a>

**Introducing Python for Electrochemistry Research**
ECS Webinar by Weiran Zheng • Aug. 28, 2024
[View on ECS Webinar Archive →](https://www.electrochem.org/ecs-webinar-archive)

### Fundamental Principles

- [Electrochemistry 101 (University of Cambridge)](https://www.ceb.cam.ac.uk/research/groups/rg-eme/research/undergraduate-teaching-notes) - Comprehensive undergraduate teaching materials
- [Electrochemical Energy Systems (MIT OpenCourseWare)](https://ocw.mit.edu/courses/chemical-engineering/10-626-electrochemical-energy-systems-spring-2014/study-materials/) - Advanced electrochemical concepts
- [Standard Operating Procedures for Cyclic Voltammetry](https://sop4cv.com/) - Practical electrochemistry guidelines

### Advanced Methods & Applications

- [Gamry Instruments Application Notes](https://www.gamry.com/application-notes/) - Technical guides for electrochemical measurements
- [Pine Research Theory & Applications](https://pineresearch.com/shop/kb/knowledge-category/theory/) - In-depth electrochemical theory
- **Reference Text:** ["Electrochemical Methods: Fundamentals and Applications"](https://www.wiley.com/en-kr/Electrochemical+Methods:+Fundamentals+and+Applications,+2nd+Edition-p-9780471043720) by Bard, Faulkner, White & Leddy - The electrochemistry bible
`;
